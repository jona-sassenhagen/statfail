...
The simulation highlights the subtlety of the pragmatic mistake in using hypothesis testing on a covariate: the test examines whether the covariate itself differs between groups (i.e. it treats the covariate as a dependent variable with group as the independent variable) and *not* whether that covariate has an impact on the dependent measure (i.e. whether there is a main effect for the covariate when it is treated as an independent variable) that may or may nor differ between groups (i.e. whether there is an interaction for the covariate when they are treated as independent variables).
How well the results of these two different tests correlate with each other is dependent on the strength of the correlation between the feature of interest and the dependent variable.

In particular, we find that when this correlation is not perfect, testing covariates can lead to unnecessary rejections of manipulations as "confounded" in 50% or more of studies for even large effects.^[20 items each for 2 groups, Cohen's $d = 2$ for manipulation, (partial) Pearson's $r = 0.75$ between feature and dependent variable; rejection rate > 60% with Cohen's $d = 1$ for covariate between groups, rejection rate = 75% with Cohen's $d = 2$ for covariate between groups][^alphabeta]

[^alphabeta]: We avoid discussing Type-I and Type-II errors here because it is not clear which hypothesis these would refer to: the hypothesis actually being tested (difference in covariate between groups) or the hypothesis of interest (confounding due to said difference). Following from the discussion above, the notion of error rate for tests on covariates is not exactly well-formed (what do we mean with "error rate" for a perfectly sampled population?). On the other hand, applying the notion to the actual hypothesis of interest seems to encourage conflating the two different hypotheses.]

The results of this simulation for various parameter settings (e.g. effect size, confound size, etc.) are available online on [RPubs (http://rpubs.com/palday/statfail)](http://rpubs.com/palday/statfail), while an interactive version is available online at [ShinyApps (https://palday.shinyapps.io/statfail/)](https://palday.shinyapps.io/statfail/). All source code (in R) is available on [GitHub (https://github.com/jona-sassenhagen/statfail)](https://github.com/jona-sassenhagen/statfail), including the ability to run the simulation on a local computer.
